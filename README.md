...
```
● ​It is based on the fact that a transaction that does not contain any frequentk-itemsets cannot contain any frequent (k + 1) - itemsets. Therefore, such a transaction can be removed from further consideration.
...
● ​Then from Lk we calculate Lk+1 frequent itemsets by generating candidates and counting them in the transaction list.
...
```
● ​When scanning each transaction in the database to generate the frequent 1-itemsets, L1 we can generate all the frequent 2-itemsets for each transaction, hash them into the different buckets of a hash table structure, and increase the corresponding bucket counts. A 2-itemset with a corresponding bucket count in the hash table that is below the support threshold cannot be frequent and thus should be removed from the candidate set.

● ​We iterate through all the transitions in the database and remove the 1 -itemsets which are not frequent from each transaction. For each transaction we then form all the 2 - ​candidates and hash them into the table with the help of hash function and increment their count.

● ​Hash Function : ​(​sum of ascii values of both items)%p , p - > Prime Number. Eg -> Hash(Milk,Jam) , p =7 => 5
